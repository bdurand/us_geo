...
class AddShortNameToCoreBasedStatisticalAreas < ActiveRecord::Migration[5.0]
  ...
  def up
    ...
    select_all("SELECT id, name FROM us_geo_core_based_statistical_areas").each do |row|
      city, state = row["name"].split(", ", 2)
      short_name = "#{city.split("-").first}, #{state.split("-").first}"
      update("UPDATE us_geo_core_based_statistical_areas SET short_name = ? WHERE id = ?", short_name, row["id"])
    end

    change_column_null :us_geo_core_based_statistical_areas, :short_name, false
    add_index :us_geo_core_based_statistical_areas, :short_name, unique: true, algorithm: :concurrently
  end
