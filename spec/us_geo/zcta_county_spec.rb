...
describe USGeo::ZctaCounty do
  ...
  it "should return the percentage of the population of the zcta is in the county" do
    zcta = USGeo::Zcta.new(population: 20_000)
    zcta_county = zcta.zcta_counties.build(population: 5000)
    expect(zcta_county.percent_population).to eq 0.25
  end

  it "should return the percentage of the land area of the zcta is in the county" do
    zcta = USGeo::Zcta.new(land_area: 200)
    zcta_county = zcta.zcta_counties.build(land_area: 50)
    expect(zcta_county.percent_land_area).to eq 0.25
  end

  it "should return the percentage of the total area of the zcta is in the county" do
    zcta = USGeo::Zcta.new(land_area: 150, water_area: 50)
    zcta_county = zcta.zcta_counties.build(land_area: 30, water_area: 20)
    expect(zcta_county.percent_total_area).to eq 0.25
  end
  ...
  describe "load" do
    ...
    it "should load the fixture data" do
      ...
      stub_request(:get, "#{USGeo::BaseRecord::BASE_DATA_URI}/zcta_counties.csv.gz").to_return(body: data)
      USGeo::ZctaCounty.load!
      expect(USGeo::ZctaCounty.count).to be > 1
